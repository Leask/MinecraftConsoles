...
#include "ConsoleStructureActionPlaceContainer.h"
...
ConsoleStructureActionPlaceContainer::ConsoleStructureActionPlaceContainer()
{
...
//void ConsoleStructureActionPlaceContainer::writeAttributes(DataOutputStream *dos, UINT numAttrs)


void ConsoleStructureActionPlaceContainer::getChildren(vector<GameRuleDefinition *> *children)
{
	ConsoleStructureActionPlaceBlock::getChildren(children);
	for(auto & item : m_items)
		children->push_back( item );
}

GameRuleDefinition *ConsoleStructureActionPlaceContainer::addChild(ConsoleGameRules::EGameRuleType ruleType)
{
	...
	else
	{
		...
		wprintf(L"ConsoleStructureActionPlaceContainer: Attempted to add invalid child rule - %d\n", ruleType );
#endif
	}
	return rule;
}

void ConsoleStructureActionPlaceContainer::addAttribute(const wstring &attributeName, const wstring &attributeValue)
{
	if(attributeName.compare(L"facing") == 0)
	{
		int value = _fromString<int>(attributeValue);
		m_data = value;
		app.DebugPrintf("ConsoleStructureActionPlaceContainer: Adding parameter facing=%d\n",m_data);
	}
	else
	{
...
bool ConsoleStructureActionPlaceContainer::placeContainerInLevel(StructurePiece *structure, Level *level, BoundingBox *chunkBB)
{
	...
	if ( chunkBB->isInside( worldX, worldY, worldZ ) )
	{
		...
		app.DebugPrintf("ConsoleStructureActionPlaceContainer - placing a container at (%d,%d,%d)\n", worldX, worldY, worldZ);
